...
#ifndef CUDA_SAFE_EXEC
#define CUDA_SAFE_EXEC(err)  __checkCudaErrors (err, __FILE__, __LINE__)
...
inline void __checkCudaErrors(CUresult err, const char *file, const int line)
{
    if (CUDA_SUCCESS != err)
    {
        fprintf(stderr, "CUDA_SAFE_EXEC() Driver API error = %04d \"%s\" from file <%s>, line %i.\n",
...
inline int gpuDeviceInitDRV(int ARGC, const char **ARGV)
{
    ...
    if (CUDA_SUCCESS == err)
    {
        CUDA_SAFE_EXEC(cuDeviceGetCount(&deviceCount));
    }
    ...
    CUDA_SAFE_EXEC(cuDeviceGet(&cuDevice, dev));
    char name[100];
...
inline int gpuGetMaxGflopsDeviceIdDRV()
{
    ...
    cuInit(0);
    CUDA_SAFE_EXEC(cuDeviceGetCount(&device_count));
    ...
    {
        CUDA_SAFE_EXEC(cuDeviceComputeCapability(&major, &minor, current_device));
    ...
    while (current_device < device_count)
    {
        CUDA_SAFE_EXEC(cuDeviceGetAttribute(&multiProcessorCount,
                                             CU_DEVICE_ATTRIBUTE_MULTIPROCESSOR_COUNT,
                                             current_device));
        CUDA_SAFE_EXEC(cuDeviceGetAttribute(&clockRate,
...
inline int gpuGetMaxGflopsGLDeviceIdDRV()
{
    ...
    while (current_device < device_count)
    {
        CUDA_SAFE_EXEC(cuDeviceGetName(deviceName, 256, current_device));
        CUDA_SAFE_EXEC(cuDeviceComputeCapability(&major, &minor, current_device));

#if CUDA_VERSION >= 3020
        CUDA_SAFE_EXEC(cuDeviceGetAttribute(&bTCC,  CU_DEVICE_ATTRIBUTE_TCC_DRIVER, current_device));
#else
...
inline CUdevice findCudaDeviceDRV(int argc, const char **argv)
{
    ...
    else
    {
        ...
        CUDA_SAFE_EXEC(cuDeviceGet(&cuDevice, devID));
        cuDeviceGetName(name, 100, cuDevice);
...
inline bool checkCudaCapabilitiesDRV(int major_version, int minor_version, int devID)
{
    ...
    CUDA_SAFE_EXEC(cuDeviceGet(&cuDevice, devID));
    CUDA_SAFE_EXEC(cuDeviceGetName(name, 100, cuDevice));
    CUDA_SAFE_EXEC(cuDeviceComputeCapability(&major, &minor, devID));
